...
abstract class Template {
   int common(int min, int max) {
      ...
      for (int i = min; i <= max; i++) {
    	   sum += apply(i); //apply method is returning value as per its Caller class (new Ma() will 
    	                    //bound to return i and new M2() will return i*i as apply() method is overridern
      }
      return sum;
   }
